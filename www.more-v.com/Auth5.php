...
//        }
    ...
    /**
      * 检查权限,group_id为0时，为超级管理员(超级管理员返回true,其他返回权限列表)
    ...
    public function check($name, $uid, $type=1, $mode='url', $relation='or') {
        if (!$this->_config['AUTH_ON'])
            return true;
        $re = $this->getAuthList($uid,$type); //获取用户需要验证的所有有效规则列表
...
//            ->where("a.uid='$uid'")
//            ->join(" left join ".$this->_config['AUTH_GROUP']." g on a.group_id=g.id")
//            ->field('uid,group_id,title,rules')->select();

        $user_groups = \think\Db::name($this->_config['AUTH_GROUP_ACCESS'])
            ->alias('a')
            ->join(  $this->_config['AUTH_GROUP']." g", "g.id=a.group_id",'LEFT')
            ->where("a.uid='$uid'")
            ->field('uid,group_id,title,rules')->select();
